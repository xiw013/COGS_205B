...
class BayesFactor:
    ...
    def evidence_slab(self):
        ...
        The integral of theta^k * (1-theta)^(n-k) from 0 to 1 is Beta(k+1, n-k+1).
        The Beta function B(x, y) = Gamma(x)Gamma(y)/Gamma(x+y).
        Evidence = choose(n, k) * B(k+1, n-k+1) = n!/(k!(n-k)!) * (k!(n-k)!)/((n+1)!)
        Evidence = 1 / (n + 1).
        """
        try:
            return 1.0 / (self.n + 1)
